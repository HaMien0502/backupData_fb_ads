...
const callAPIInsightsAd = async (adAccountId) => {
    const now = new Date();
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        try {
            const response = await axios.get(urlInsights, {
                params: {
                    fields: "account_id,account_name,campaign_id,campaign_name,adset_id,adset_name,ad_id,ad_name,impressions,clicks,spend,actions,action_values,ctr,cpc,reach,conversion_values",
                    level: "ad",
...
const LARK_API_FB_ADS = `https://open.larksuite.com/open-apis/bitable/v1/apps/${process.env.LARK_APP_TOKEN_FB_ADS}/tables/${process.env.LARK_TABLE_ID_INSIGHT_AD}/records`;
...
const convertDataForCheck = (data) => {
    return {
        fields: {
            ...
            ad_id: data.fields.ad_id,
            ad_name: data.fields.ad_name,
    ...
    for (let i = 0; i < listAdsAccounts_metadevlopers.length; i++) {
        ...
        for (let j = 0; j < listAdsAccounts_lark.length; j++) {
            ...
            if (dataLB.fields.date_start == dataDevloper.date_start && dataLB.fields.date_stop == dataDevloper.date_stop && dataLB.fields.campaign_id == dataDevloper.campaign_id
                && dataLB.fields.adset_id == dataDevloper.adset_id && dataLB.fields.ad_id == dataDevloper.ad_id
            ) {
                break;
            };
...
const sendLarkAdsAdsetNew = async (fields) => {
    try {
        return await axios.post(
            `https://open.larksuite.com/open-apis/bitable/v1/apps/${process.env.LARK_APP_TOKEN_FB_ADS}/tables/${process.env.LARK_TABLE_ID_INSIGHT_AD}/records`,
            { fields },
            {
                headers: {
                    "Content-Type": "application/json",
...
const getAdLevelReport = async () => {
    const listAdsAccounts_metadevlopers = await callAPIInsightsAd("act_439558778837010");
...
module.exports = getAdLevelReport;
